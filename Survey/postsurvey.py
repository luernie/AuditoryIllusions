...
class PostSurveyApp(tk.Tk):
    ...
    def _slider_row(self, lo_label, hi_label):
        """Canvas-based slider with tick marks, '1'/'10' labels, and jump-to-click."""
        SLIDER_W = 320
        TRACK_Y  = 18
        TICK_H   = 7
        THUMB_R  = 7
        ...
        tk.Label(outer, text=lo_label, font=("Arial", 9), fg="#444",
                 bg="#f5f5f5", wraplength=130, justify="right", width=16).pack(side="left")
        ...
        tk.Label(outer, text=hi_label, font=("Arial", 9), fg="#444",
                 bg="#f5f5f5", wraplength=130, justify="left", width=16).pack(side="left")
        ...
        def val_to_x(v):
            return track_x0 + (v - 1) / 9 * track_len

        def x_to_val(x):
            return max(1.0, min(10.0, 1 + (x - track_x0) / track_len * 9))

        canvas.create_line(track_x0, TRACK_Y, track_x1, TRACK_Y,
                           fill="#aaa", width=2)
    ...
    def _build_ui(self):
        ...
        tk.Label(self.frame, text="Rate each dimension based on your overall experience during the experiment.",
        ...
        self._label("Temporal Demand", "How much time pressure did you feel while completing the task?")
        ...
        self._label("Effort", "How hard did you have to work to achieve your level of performance?")
        self.effort = self._slider_row("Very Low Effort", "Very High Effort")

        self._label("Frustration", "How irritated, stressed, or annoyed did you feel during the task?")
        self.frustration = self._slider_row("Not at All", "Very Much")

        self._label("Confidence", "How confident were you in your judgments during the task?")
        ...
        # Submit
        tk.Button(
            self.frame, text="Submit Survey", font=("Arial", 11, "bold"),
    ...
    def _submit(self):
        ...
        data = {
            ...
            "Mental Demand (1-10)": round(self.mental.get(), 2),
            "Temporal Demand (1-10)": round(self.temporal.get(), 2),
            "Performance (1-10)": round(self.performance.get(), 2),
            "Effort (1-10)": round(self.effort.get(), 2),
        ...
        save_response(data)
        messagebox.showinfo("Saved", f"Response for participant '{pid}' saved to {OUTPUT_FILE}.")
        self.destroy()
